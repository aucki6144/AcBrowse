...
public class BookMarks implements CusUrlList {

    protected String SHARED_PREFERENCE_KEY = "getBookMarkStorage";
    ...
    public void deleteItem(Context context, CusUrl cusUrl) {
        String str = cusUrl.urlString();
        String jsonData = this.getJsonList(context);
        if(jsonData != "NONE"){
            List<CusUrl> list = translateJsonToList(jsonData);
            for (int index=0;index<list.size();index++){
                if (list.get(index).urlString().equals(str)){
                    list.remove(index);
